...
usage() {
  echo "Usage: $0 [ -d DISPLAY_NAME ] [ -i APP_IDENTIFIER ] [ -r SPA_REDIRECT ] [ -m ] [ -s ] [ -h ] [ -p ] [ -c ] [ -o ]
...
while getopts "shd:i:r:pmco" options; do  
    case "${options}" in
        d)
            DISPLAY_NAME=${OPTARG}
            ;;
        i)
            APP_IDENTIFIER=${OPTARG}
            ;;
        h)
            usage
            exit 0
...
then
    ...
    AADAPPREG=$(az ad app create --display-name $DISPLAY_NAME --available-to-other-tenants false)
else
...
then
    ...
    az ad app update --id $APPID --identifier-uris $APP_IDENTIFIER
fi
...
then
    ...
    APPCRED=$(az ad app credential reset --id $APPID --credential-description "Autogen" --only-show-errors)
...
then
    ...
    # Disable default scope first
    DEFAULT_SCOPE=$(az ad app show --id $APPID | jq '.oauth2Permissions[0].isEnabled = false' | jq -r '.oauth2Permissions')
    az ad app update --id $APPID --set oauth2Permissions="$DEFAULT_SCOPE" --only-show-errors

    # Set scopes based on JSON file
    az ad app update --id $APPID --set oauth2Permissions=@read-write-scope.json --only-show-errors
fi
